...
void fdbar(Dbar *dbar, FILE *stream) {
  ...
  if (dbar->mode == textual) {
    ...
    case outlined:
      if (dbar->segb == 0) {
        fprintf(stream,
                "%s^ib(1)^fg(%s)^ro(%dx%d)^p(%d)^fg(%s)^r(%dx%d)^p(%d)^ib(0)^"
                "fg()%s",
                dbar->label ? dbar->label : "", dbar->bg, (int)dbar->width,
                dbar->height, -1 * (dbar->width - 2), dbar->fg,
                (int)l > dbar->width - 4 ? dbar->width - 4 : (int)l,
                dbar->height - 4 > 0 ? dbar->height - 4 : 1,
        ...
        printf("^fg()^ib(0)^p(%d)%s",
               dbar->width - i * (dbar->segw + dbar->segb),
               dbar->pnl ? "\n" : "");
      }
      break;
    ...
    case graph:
      ...
      for (i = dbar->gc + 1;
           i < MAX_GRAPH_VALS && (i * (dbar->gs + dbar->gw)) < dbar->width;
           ++i) {
        p = 100 * dbar->gb[i] / dbar->height;
        p = (int)p + 0.5 >= (int)p ? (int)(p + 0.5) : (int)p;
        fprintf(stream, "^fg(%s)^p(%d)^r(%dx%d+0-%d)", dbar->fg, dbar->gs,
                dbar->gw, (int)dbar->gb[i], (int)dbar->gb[i] + 1);
      }
    ...
    case pie:
      fprintf(stream, "^ib(1)^fg(%s)^c(%d)^p(-%d)^fg(%s)^c(%d-%d)%s", dbar->bg,
              dbar->width, dbar->width, dbar->fg, dbar->width,
              (int)(rp * 360 / 100), dbar->pnl ? "\n" : "");
      break;
    ...
    default:
      if (dbar->segb == 0)
        ...
        printf("%s^fg(%s)^r(%dx%d)^fg(%s)^r(%dx%d)^fg()%s",
               dbar->label ? dbar->label : "", dbar->fg, (int)l, dbar->height,
        ...
        for (i = 0; i < segs; i++) {
          if (i < segsa)
            fprintf(stream, "^fg(%s)^r(%dx%d+%d+%d)", dbar->fg, dbar->segw,
                    dbar->height, i ? dbar->segb : 0, 0);
          else
            fprintf(stream, "^fg(%s)^r(%dx%d+%d+%d)", dbar->bg, dbar->segw,
                    dbar->height, i ? dbar->segb : 0, 0);
        }
