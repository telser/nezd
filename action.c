...
struct event_lookup ev_lookup_table[] = {
    {"onstart", onstart},       {"onexit", onexit},
    {"onnewinput", onnewinput}, {"button1", button1},
    {"button2", button2},       {"button3", button3},
    {"button4", button4},       {"button5", button5},
    {"button6", button6},       {"button7", button7},
    {"entertitle", entertitle}, {"leavetitle", leavetitle},
    {"enterslave", enterslave}, {"leaveslave", leaveslave},
    {"sigusr1", sigusr1},       {"sigusr2", sigusr2},
    {"keymarker", keymarker},   {0, 0}};
...
int get_ev_id(const char *evname) {
  ...
  for (i = 0; ev_lookup_table[i].name; i++) {
    if (strncmp(ev_lookup_table[i].name, evname,
                strlen(ev_lookup_table[i].name)) == 0)
...
int a_togglecollapse(char *opt[]) {
  ...
  if (nezd.slave_win.max_lines &&
      (XGetWindowAttributes(nezd.dpy, nezd.slave_win.win, &wa),
...
int a_unhide(char *opt[]) {
  ...
  if (nezd.title_win.ishidden) {
    if (!nezd.slave_win.ishmenu)
      XResizeWindow(nezd.dpy, nezd.title_win.win, nezd.title_win.width,
                    nezd.line_height);
    else
      XResizeWindow(nezd.dpy, nezd.slave_win.win, nezd.title_win.width,
                    nezd.line_height);
...
int a_togglehide(char *opt[]) {
  ...
  nezd.title_win.ishidden ? a_unhide(NULL) : a_hide(opt);
...
int a_menuprint(char *opt[]) {
  ...
  if (nezd.slave_win.ismenu && nezd.slave_win.sel_line != -1 &&
      (nezd.slave_win.sel_line + nezd.slave_win.first_line_vis) <
          nezd.slave_win.tcnt) {
...
int a_grabkeys(char *opt[]) {
  (void)opt;
  XGrabKeyboard(nezd.dpy, RootWindow(nezd.dpy, nezd.screen), True,
...
int a_grabmouse(char *opt[]) {
  (void)opt;
  XGrabPointer(nezd.dpy, RootWindow(nezd.dpy, nezd.screen), True,
